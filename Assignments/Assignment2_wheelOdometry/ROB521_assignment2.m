...
for i = 2:numodom
    ...
    delta_theta = omega_odom(i) * delta_t;
    theta_odom(i) = theta_odom(i-1) + delta_theta;
    
    delta_dist = v_odom(i) * delta_t;
    delta_x = delta_dist * cos(theta_odom(i));
    delta_y = delta_dist * sin(theta_odom(i));
    
    x_odom(i) = delta_x + x_odom(i-1);
    y_odom(i) = delta_y + y_odom(i-1);
end
...
for n=1:100
    ...
    for i = 2:numodom
        delta_t = t_odom(i) - t_odom(i-1);
        delta_theta = delta_t * omega_odom(i);
        theta_odom(i) = theta_odom(i-1) + delta_theta;
        ...
        x_odom(i) = x_odom(i-1) + delta_x;
        y_odom(i) = y_odom(i-1) + delta_y;
    end
...
for n=1:2
    ...
    % ------insert your point transformation algorithm here------
        if abs(omega_interp(i)) < 0.1 
            x_local = (y_laser(i, :) .* cos_angles) + 0.1; 
            y_local = y_laser(i, :) .* sin_angles;

            x_global = cos(theta_interp(i)) * x_local - sin(theta_interp(i)) * y_local;
            y_global = sin(theta_interp(i)) * x_local + cos(theta_interp(i)) * y_local;

            x_global = x_global + x_interp(i);
            y_global = y_global + y_interp(i);

            if n == 1
                plot(x_global, y_global, 'r.');
            else
                plot(x_global, y_global, 'b.');
            end
        end
