...
% =========
%
% This assignment will in   troduce you to the idea of first building an
    ...
    for n=1:nparticles
        ...
        for beam=1:2         
            ...
            if ~isnan(y_laser(i,j)) && y_laser(i,j) < y_laser_max
                ...
                threshold = 0.5;
                y_laser_pred = predict_laser_range(row_pos, col_pos, theta_particle(n), angles(j), ogp, ogres, threshold);
                w_particle(n) = w_particle(n)*w_gain*pdf(y_laser(i, j), y_laser_pred, sqrt(laser_var));
