...
%    Question 2: see ass3_q2.mbresenham_line
%
...
for i=1:5:size(t_laser,1)
    ...
    x_pos = x_interp(i);  
    y_pos = y_interp(i);  
    theta_pos = theta_interp(i);

    x_start = round((x_pos - ogxmin) / ogres);
    y_start = round((y_pos - ogymin) / ogres);
  
    for j = 1:npoints
        if ~isnan(y_laser(i,j)) && r_min_laser < y_laser(i,j) && y_laser(i,j) < r_max_laser
            
            range_mes = y_laser(i,j);
            cur_angle = angles(j) + theta_pos;
            x_end = x_start + round(range_mes * cos(cur_angle) / ogres);
            y_end = y_start + round(range_mes * sin(cur_angle) / ogres);

            [rr, cc] = ray_trace(x_start, y_start, x_end, y_end);

            for k = 1:length(rr)
                if rr(k) > 0 && rr(k) <= ogny && cc(k) > 0 && cc(k) <= ognx
                    if k < length(rr)
                        oglo(rr(k),cc(k)) = oglo(rr(k),cc(k)) - 0.5;
                    else
                        oglo(rr(k),cc(k)) = oglo(rr(k),cc(k)) + 3;
                    end
                end
            end
        end
