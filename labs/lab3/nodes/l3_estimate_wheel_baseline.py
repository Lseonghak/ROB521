...
class wheelBaselineEstimator():
    ...
    def startStopCallback(self, msg):
        # print('startStopCallback : ', msg.angular.z)
        if self.isMoving is False and np.absolute(msg.angular.z) > 0:
            ...
            total_l_dist = abs(self.del_left_encoder / TICKS_PER_ROTATION) * (2 * np.pi * WHEEL_RADIUS)
            total_r_dist = abs(self.del_right_encoder / TICKS_PER_ROTATION) * (2 * np.pi * WHEEL_RADIUS)

            avg_distance = (total_l_dist + total_r_dist) / 2
            
            baseline = avg_distance / np.pi
