...
class OccupancyGripMap:
    ...
    def scan_cb(self, scan_msg):
        ...
        robot_x = odom_map[0] / CELL_SIZE  
        robot_y = odom_map[1] / CELL_SIZE 
        robot_orientation = odom_map[2]
        
        for i, range_mes in enumerate(scan_msg.ranges):
            angle = scan_msg.angle_min + i * scan_msg.angle_increment + robot_orientation
            if range_mes == float('inf') or range_mes == 0.0:
                continue
            self.ray_trace_update(self.np_map, self.log_odds, int(robot_x), int(robot_y), angle, range_mes)
    ...
    def ray_trace_update(self, map, log_odds, x_start, y_start, angle, range_mes):
        ...
        for y, x in zip(rr[:-NUM_PTS_OBSTACLE], cc[:-NUM_PTS_OBSTACLE]):
            if 0 <= x < map.shape[1] and 0 <= y < map.shape[0]:
                log_odds[y, x] -= ALPHA
                map[y, x] = max(0, min(100, self.log_odds_to_probability(log_odds[y, x]) * 100))
